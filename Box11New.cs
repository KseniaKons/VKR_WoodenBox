...
namespace WoodenBox
{
    internal class Box11New
    {
        ...
        void NewShield(double height, double width, double length, double col, string name, string foldername)
        {
        ...
        { //дно бок торец
            ...
            //ЩИТ дна и крышки
            NewShield(heightBoard, w_fact_bottom, y + 4 * heightBoard, col_bottom, name_bottom, foldername); // передать параметры досок  ширина высота длинна

            //боковой щит
            NewShield(heightBoard, w_fact_side, y + 4 * heightBoard, col_side, name_side, foldername);

            //торцевой щит
            NewShield(heightBoard, w_fact_side, lenghtBT, col_side, name_before, foldername);
