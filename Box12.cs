...
namespace WoodenBox
{
    internal class Box12
    {
        ...
        { //дно бок торец
            ...
            number++;
            number++;
            NewShield(heightBoard, w_fact_bottom, y + 4 * heightBoard, col_bottom, gap, name_bottom, 
                foldername, marking, CL_bottom, numDesignation, true); // передать параметры досок  ширина высота длинна

            //торцевой щит
            MarkingBox(number, out numDesignation);
            number++;
            NewShield(heightBoard, w_fact_side, lenghtBT, col_side, gap, name_before, 
                foldername, marking, CL_before, numDesignation, true);

            //боковой щит
            MarkingBox(number, out numDesignation);
            number++;
            NewShield(heightBoard, w_fact_side, y + 4 * heightBoard, col_side, gap, name_side,
                foldername, marking, CL_side, numDesignation, true);
            ...
            //дно и бок 2, бок 1
            ksDoc3d1.AddMateConstraint(0, namePlane[4], namePlane2[0], -1, 1, 0);
            ksDoc3d1.AddMateConstraint(5, namePlane1[3], namePlane2[3], 1, 1, w_fact_bottom * col_bottom - heightBoard + gap * (col_bottom - 1));
            ksDoc3d1.AddMateConstraint(0, namePlane[2], namePlane2[2], 1, 1, 0);
            ...
            ksDoc3d1.AddMateConstraint(0, namePlane1[4], namePlane3[0], 1, 1, 0);
            ksDoc3d1.AddMateConstraint(0, namePlane1[2], namePlane3[2], 1, 1, 0);
            ksDoc3d1.AddMateConstraint(5, namePlane1[0], namePlane3[4], -1, 1, -(w_fact_side * col_side + heightBoard + gap * (col_side - 1)));
